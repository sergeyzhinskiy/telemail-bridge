<#
.SYNOPSIS
    TeleMail Bridge – финальная установка на Windows 10/11
.DESCRIPTION
    Использует Python 3.11, PostgreSQL 16, Redis.
...
$GIT_REPO = "https://github.com/sergeyzhinskiy/telemail-bridge.git"   # ← укажите свой репозиторий
...
# Глобальные переменные (заполняются по ходу скрипта)
...
function Test-SystemRequirements {
    ...
    if ($freeSpace -lt 12) {
        Write-ErrorMsg "Not enough disk space on C:. Need ~12 GB. You have: $freeSpace GB"
    }
...
function Install-Python311 {
    ...
    # Поиск уже существующего Python 3.11
    $searchPaths = @(
    ...
    foreach ($p in $searchPaths) {
        if ($p -and (Test-Path $p)) { $script:PythonExe = $p; break }
    }
    if (-not $script:PythonExe) {
        try { $script:PythonExe = (py -3.11 -c "import sys; print(sys.executable)") } catch {}
    }
    if ($script:PythonExe) {
        $ver = & $script:PythonExe --version 2>&1
        Write-Success "Python 3.11 found: $ver (path: $script:PythonExe)"
        return
    }
    ...
    if (Test-Path $installer) {
        if ((Get-Item $installer).Length -lt 20MB) { Remove-Item $installer -Force }
    }
    if (-not (Test-Path $installer)) {
        Invoke-WebRequest -Uri $pythonUrl -OutFile $installer -TimeoutSec 300
    }
    ...
    if (-not $script:PythonExe) {
        Write-ErrorMsg "Python 3.11 installed but python.exe not found!"
    }
...
function Install-SystemPackages {
    ...
    if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
        Write-ErrorMsg "Git installation failed. Install manually and rerun the script."
    }
...
function Install-PostgreSQL {
    ...
    & "$pgBin\psql.exe" -U postgres -c "CREATE USER telemail WITH PASSWORD '$script:PG_PASSWORD';" 2>$null
...
# ------------------------------ Подготовка окружения и клонирование ------------------------------
function Prepare-Environment {
    Write-Step "5/7 Preparing directories & cloning repository"

    # Создаём основные папки
    $dirs = @($APP_ROOT, $LOG_DIR, $DATA_DIR, $CONFIG_DIR,
              "$DATA_DIR\sessions", "$DATA_DIR\media", "$DATA_DIR\temp", "$DATA_DIR\backups")
    foreach ($d in $dirs) {
    ...
    if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
        Write-ErrorMsg "Git is not installed. Please install Git and run the script again."
    }

    # Удаляем старую папку src, если есть
    if (Test-Path "$APP_ROOT\src") {
        Write-Info "Removing existing src directory..."
        Remove-Item "$APP_ROOT\src" -Recurse -Force -ErrorAction Stop
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-ErrorMsg "Git clone failed. Check repository URL and internet connection."
    }
    if (-not (Test-Path "$APP_ROOT\src\.git")) {
        Write-ErrorMsg "Repository not cloned properly."
    }
...
function Setup-PythonVenv {
    ...
    # Создаём venv строго с найденным Python 3.11
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-ErrorMsg "aiohttp failed to install. Make sure Python 3.11 is used and C++ Build Tools are installed."
    }
...
function Configure-App {
    ...
    $script:AdminPassword = $adminPassword   # сохраняем глобально
    ...
    $script:AdminEmail = $ADMIN_EMAIL   # сохраняем глобально

    # Убедимся, что папка src существует
    if (-not (Test-Path "$APP_ROOT\src")) {
...
function Initialize-Database {
    ...
    # Используем сохранённые глобальные переменные
...
function Create-WindowsServices {
    ...
    function Create-SCService($Name, $Display, $Command, $Args) {
        ...
        $binPath = "`"$Command`" $Args"
        $res = sc.exe create $Name binPath= "$binPath" start= auto DisplayName= "$Display"
        if ($LASTEXITCODE -ne 0 -and $res -notmatch "already exists|success") {
            Write-Warning "sc create $Name output: $res"
        } else {
    ...
    Write-Success "Services creation completed"
}
    ...
    foreach ($svc in $services) {
        ...
        if ($s) {
            if ($s.Status -ne "Running") {
                ...
                Write-Success "$svc is already running"
            }
        } else {
